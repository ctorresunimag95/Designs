# AspireTemplate Architecture & Services

This document describes the distributed services and agents that make up the AspireTemplate application ecosystem.

## Overview

AspireTemplate is composed of multiple independent services coordinated through a .NET Aspire AppHost, with Azure Services providing the underlying infrastructure for messaging, storage, and configuration.

## Core Services

### 1. AppHost (`infrastructure/AspireTemplate.AppHost`)
...
### 2. SampleApi (`src/AspireTemplate.SampleApi`)
...
### 3. Data Layer (`src/AspireTemplate.Data`)
...
### 4. Azure Functions (`src/AspireTemplate.SampleFunction`)
...
### 5. Migration Service (`infrastructure/AspireTemplate.MigrationService`)
...
### 6. Service Defaults (`infrastructure/AspireTemplate.ServiceDefaults`)
...
1. **AppHost Startup:** `dotnet run --project infrastructure/AspireTemplate.AppHost`
