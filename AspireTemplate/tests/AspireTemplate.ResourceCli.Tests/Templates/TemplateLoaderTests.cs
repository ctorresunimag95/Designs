...
namespace AspireTemplate.ResourceCli.Tests.Templates;
...
public class TemplateLoaderTests
{
    ...
    {
        var content = TemplateLoader.LoadRegion(key);
        Assert.Contains($"// <AspireTemplate resource=\"{key}\">", content, StringComparison.Ordinal);
    }
    ...
    {
        var content = TemplateLoader.LoadRegion(key);
        Assert.Contains("// </AspireTemplate>", content, StringComparison.Ordinal);
    }
    ...
    [Fact]
    public void RealmExportJson_Should_ContainAspireTemplateRealm()
    {
