...
```bash
dotnet user-secrets set "Parameters:SqlPassword" "YourLocalPassword!" --project infrastructure/AspireGuide.AppHost
```
...
```bash
dotnet ef migrations add <MigrationName> --project src/AspireGuide.Data --startup-project src/AspireGuide.SampleApi
```
